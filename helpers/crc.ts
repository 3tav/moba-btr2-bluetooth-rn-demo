...
const vCRC32 = [0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3, 0x0EDB8832,
...
const reverseBytes = (num: number): number => {
  return ((num & 0xFF) << 24) |
    ((num & 0xFF00) << 8) |
    ((num >> 8) & 0xFF00) |
    ((num >> 24) & 0xFF);
...
export const crc32Calc = (dataBuffer: Buffer): number => {
  const len = dataBuffer.length;
  let c = 0;

  for (let i = 0; i < len; i++) {
    c = (c >>> 8) ^ vCRC32[dataBuffer.readUint8(i) ^ (c & 0x000000FF)];
  }
  return reverseBytes(c) >>> 0;
}

export const calculateCRC32OnReceivedBtr2Data = (data: Buffer): string => {
  // find index of 0x03
